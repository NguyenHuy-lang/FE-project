      ...
      if (data.length !== 0) {
        let render1 = data.listProductOrder.map((item, index) => {
          return `<tr>
                  ...
                  <td id="${index + 1}-id-product" class="product-name">${
            item.product.name
          } </td>
                  <td id="${index + 1}-id-product" class="product-price">${
            item.price
          } </td>
                  <td id="${index + 1}-id-product" class="product-quantity">${
            item.quantity
          } </td>
                  <td id="${index + 1}-id-product" class="product-total">${
...
const deleteProduct = (productId) => {
  ...
  fetch(APIurl, options).then((data) => {
